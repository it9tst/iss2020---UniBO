...
class Test01 {
	...
	fun testStateWaiter(){
	 	runBlocking{
			...
			MsgUtil.sendMsg(MsgUtil.buildRequest("smartbell","enter_request_client","enter_request_client(36)","smartbell"),smartbell!!)
			delay(15000)
			MsgUtil.sendMsg(MsgUtil.buildDispatch("waitermind","client_ready_to_order","client_ready_to_order(1, brioches)","waitermind"),waitermind!!)
			delay(45000)
			
			MsgUtil.sendMsg(MsgUtil.buildDispatch("waitermind","client_payment","client_payment(0)","waitermind"),waitermind!!)
			delay(20000)
			println("TEST | start check 'tableCleared'")
			checkState("tableCleared", 1)
			println("TEST | tableCleared checked")
			println("TEST | click enter to continue")
			
			MsgUtil.sendMsg(MsgUtil.buildDispatch("waitermind","client_ready_to_order","client_ready_to_order(1, the)","waitermind"),waitermind!!)
			delay(27000)
			println("TEST | start check 'tableCleaned'")
			checkState("tableCleaned", 1)
			println("TEST | tableCleaned checked")
			println("TEST | click enter to continue")
