		...
		return { //this:ActionBasciFsm
				...
				state("waitOrder") { //this:State
					action { //it:State
					 ...
					 transition(edgeName="t012",targetState="prepareOrder",cond=whenDispatch("send_order"))
					transition(edgeName="t013",targetState="endWork",cond=whenDispatch("end"))
				}	 
				state("prepareOrder") { //this:State
					action { //it:State
						...
						if( checkMsgContent( Term.createTerm("send_order(ID,ORD)"), Term.createTerm("send_order(ID,ORD)"), 
								...
								forward("barman_complete_order", "barman_complete_order(${payloadArg(0)},${payloadArg(1)})" ,"waiter" ) 
						}
					}
					 transition( edgeName="goto",targetState="waitOrder", cond=doswitch() )
				}	 
